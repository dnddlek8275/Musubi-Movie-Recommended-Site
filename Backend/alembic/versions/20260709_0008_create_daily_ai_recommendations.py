...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_daily_ai_recommendations_recommend_date",
        "daily_ai_recommendations",
        ["recommend_date"],
        unique=True,
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "daily_recommendation_id",
            "display_order",
            name="uq_daily_ai_recommendation_movies_order",
        ),
        sa.CheckConstraint(
            "display_order BETWEEN 1 AND 3",
            name="ck_daily_ai_recommendation_movies_display_order",
        ),
    )
